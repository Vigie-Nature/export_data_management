select `observations`.`id` AS `observation_id`,
  `releves`.`id` AS `session_id`,
	...
	json_extract(`releves`.`data`,'$.transect.coordonnees') AS `transect_geometry`,
	...
	json_value(`espaces_verts`.`observations`.`data`,'$.cdNomTaxref') AS `cd_nom`,
	json_value(`espaces_verts`.`observations`.`data`,'$.abondance') AS `taxon_count`
